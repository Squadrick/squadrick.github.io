...
internally it uses either TCP/IP or UDP, which are both quite close
...
need to do an intermediate copy to the process' memory space.
...
a large chuck of system memory using the topic name as the key, and the
memory is used as a circular buffer. Each publisher (writer) writes at the
last index position, overwritting older pre-existing data. The subscribes
...
You can find the full detials of the implementation in the
[GitHub repo](https://github.com/squadrick/shadesmar), but moving onto the
title of the blog post: Interprocess Locks

### Interprocess Synchronization

Since multiple publishers and subscribes have the ability to read and write
from the same memory buffer, there's a possibility of data corruption if
the reads and writes overlap. To prevent this, each publisher acquire an
exclusive lock for the buffer, writes the data and unlock the lock. Since,
the subscribers are read-only, they acquired a shared lock on the buffer,
...
is not something we can guarentee, as simplying `Ctrl+C`ing a running process
will cause this fail.


#### Failure

If a process dies while have exclusive or shared access to `ipc_mut` and
...
check if the PID is still alive using `stat`, if it's dead it will
force acquire the lock setting it's own PID, else if it's alive, we continue
to wait for access.
...
while(!mut.try_lock()) {                 
  if (exclusive_owner != 0) {           
    if (proc_dead(exclusive_owner)) {   //<-- proc 1 (enters the if, if proc 1 run before proc 2)
...
Using `atomic`, we can guarentee that `lock` and `unlock` work even
...
a basic `interprocess_sharable_mutex`. `robust_ipc_mutex` can handle upto
